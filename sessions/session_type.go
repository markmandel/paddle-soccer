...
func (s *Server) storeSession(sess Session) error {
	...
	err := con.Send("MULTI")
	if err != nil {
		log.Printf("[Error][Session Type] Could not Send MULTI: %v", err)
	}
	...
	if err != nil {
		log.Printf("[Error][Session Type] Could not Send HMSET: %v", err)
	}
	err = con.Send("EXPIRE", key, 60*60)
	if err != nil {
		log.Printf("[Error][Session Type] Could not Send EXPIRE: %v", err)
	}
	_, err = con.Do("EXEC")

	if err != nil {
		log.Printf("[Error][Session Type] Could not save session to redis: %v", err)
	}
...
func (s *Server) getSession(id string) (Session, error) {
	...
	if err != nil {
		log.Printf("[Error][Session Type] Error getting hash for key %v. %v", key, err)
		return result, err
	}

	if len(values) == 0 {
		log.Printf("[Error][Session Type] Could not find record for key: %v", key)
	...
	if err != nil {
		log.Printf("[Error][Session Type] Error Scanning Struct %v", err)
	}
